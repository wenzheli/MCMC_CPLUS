#ifndef MCMC_LEARNING_MCMC_SAMPLER_STOCHASTIC_SPARSE_H__
#define MCMC_LEARNING_MCMC_SAMPLER_STOCHASTIC_SPARSE_H__
...
namespace mcmc {
	namespace learning {
		...
		class MCMCSamplerStochasticSparse : public SparseLearner {
		public:
			/**
			...
			*/
			MCMCSamplerStochasticSparse(const Options &args, const Network &graph)
				: Learner(args, graph) {
		...
		protected:
			void update_beta(const OrderedEdgeSet &mini_batch, double scale){
				...
				for (auto edge = mini_batch.begin(); edge != mini_batch.end(); edge++){
					...
					for (int k = 0; k < K; k++){
						double tmp1;
						double tmp2;
						map<int, double> map1 = pi.at(i);
						map<int, double> map2 = pi.at(j);
						...
						pi_sum += tmp1 * tmp2;
						probs[k] = std::pow(beta[k], y) * std::pow(1 - beta[k], 1 - y) * tmp1 * tmp2;
					}
			...
			double getSumMap(map<int, double> a, int n) const{
				double s = 0.0;
				for (int i = 0; i < n; i++){
					if (a.find(i) != a.end()){
						s += a[i];
			...
			void update_phi(int i, const OrderedVertexSet &neighbors){
				...
				//double eps_t = std::pow(1024+step_count, -0.5);
				double phi_i_sum = getSumMap(phi.at[i], K);
				...
				map<int, double> map_i = pi.at(i);
				
				map<int, double> phi_i = phi.at(i);
				...
				for (auto neighbor = neighbors.begin();
					...
					map<int, double> map_j = pi.at(*neighbor);
					...
					for (int k = 0; k < K; k++){
						double tmp1;
						double tmp2;
						if (map_i.find(k) != map_i.end()){
							tmp1 = map_i[k];
						}else{
							tmp1 = map_i[-1];
						}

						if (map_j.find(k) != map_j.end()){
							tmp2 = map_j[k];
						}else{
							tmp2 = map_j[-1];
						}

						probs[k] = std::pow(beta[k], y_ab) * std::pow(1 - beta[k], 1 - y_ab) * tmp1 * tmp2;
						probs[k] += std::pow(epsilon, y_ab) * std::pow(1 - epsilon, 1 - y_ab) * tmp2 * (1 - tmp2);
					}

					double prob_sum = getSum(probs, K);
					for (int k = 0; k < K; k++){
						double tmp;
						if (phi_i.find(k) != phi_i.end()){
							tmp = phi_i[k];
						}else{
							tmp = phi_i[-1];
						}
						grads[k] += (probs[k] / prob_sum) / tmp - 1.0 / phi_i_sum;
					}
					delete[] probs;
				}
				// update phi for node i
				double* phi_tmp = new double[K]();
				map<int, double> phi_i_new;
				for (int k = 0; k < K; k++){
					double tmp_k;
					if (phi_i.find(k) != phi_i.end()){
						tmp_k = phi_i[k];
					...
					phi_tmp[k] = std::abs(tmp_k + eps_t / 2 * (alpha - tmp_k + (N*1.0 / num_node_sample) *grads[k]) + std::pow(eps_t, 0.5)*std::pow(tmp_k, 0.5) *noise[k]);
				}

				// select the top k_max
  				std::priority_queue<std::pair<double, int>> q;
  				for (int k = 0; i < K; k++) {
    				q.push(std::pair<double, int>(phi_tmp[k], k));
  				}
  				
  				for (int i = 0; i < max_k; ++i) {
    				int ki = q.top().second;
    				phi_i_new[ki] = phi_tmp[ki];
    				q.pop();
    			}

    			double sum = 0.0;
    			for (int i = max_k; i < K; i++){
    				int ki = q.top().second;
    				sum += phi_tmp[ki];
    				q.pop();
    			}
    			phi_i_new[-1] = sum/(K-max_k);

    			vector<map<int, double> >::iterator iter = phi.begin();
    			iter.insert(i, phi_i_new);
