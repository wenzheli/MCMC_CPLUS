#ifndef MCMC_LEARNING_LEARNER_STATE_TRANSITION_H__
#define MCMC_LEARNING_LEARNER_STATE_TRANSITION_H__
	...
	namespace learning {
		...
		*/
		...
		public:
			...
			*/
			LearnerStateTransition(const Options &args, const Network &network)
				: network(network) {
			...
			virtual ~LearnerStateTransition() {
			}
		...
		protected:
			...
			*/
			double cal_perplexity(const EdgeMap &data) {
				...
				for (EdgeMap::const_iterator edge = data.begin();
					...
					//	edge->second, beta);
					double edge_likelihood = cal_edge_likelihood(e.first, e.second, edge->second, beta);
					...
					ppx_for_heldout[idx] = edge_likelihood;
			...
			*/
			double cal_edge_likelihood(const int node_1, const int node_2,
				bool y, const double* beta) const {
				
				std::map<int, double> active_1 = pi_active[node_1]; 
				std::map<int, double> candidate_1 = pi_candidate[node_1];
				std::map<int, double> active_2 = pi_active[node_2];
				std::map<int, double> candidate_2 = pi_candidate[node_2];
				...
				if (y) {
					for (int k = 0; k < K; k++) {
						double term1;
						double term2;
						if (active_1.find(k) != active_1.end()){
							term1 = active_1[k];
						} else if (candidate_1.find(k) != candidate_1.end()){
							term1 = candidate_1[k];
						} else{
							term1 = pi_bulk[node_1];
						}

						if (active_2.find(k) != active_2.end()){
							term2 = active_2[k];
						} else if (candidate_2.find(k) != candidate_2.end()){
							term2 = candidate_2[k];
						} else{
							term2 = pi_bulk[node_2];
						}

						s += term1 * term2 * beta[k];
					}
				}
				else {
					double sum = 0.0;
		...
		protected:
			...
			std::vector<std::map<int, double> > pi_active;
			std::vector<std::map<int, double> > pi_candidate;
			std::vector<double> pi_bulk;
